...
Full audit: `docs/product/audit.md`. As of 2026-08-19: comprehensive audit and roadmap complete (12 phases, `docs/roadmap/README.md`); Phases 1-8 implemented. The codebase is materially more mature than a typical MVP — real OSRM routing, a genuinely sophisticated matching algorithm, a rich domain schema, and a production-grade driver-onboarding flow already exist. Both of the audit's two confirmed highest-priority gaps are now addressed: the fake (explicitly non-geospatial) passenger pickup-point picker was retired in Phase 5, and unconstrained pricing was replaced with a computed, bounded suggestion in Phase 6 — though Phase 6's `base_rate_per_km` is explicitly a first-cut value pending business confirmation, not a settled number (see "Important decisions" below).
