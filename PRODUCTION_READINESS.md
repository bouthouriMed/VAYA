...
**Audit date:** 2026-09-10 (two passes, same day); third pass (Terms & Conditions + account deletion, user-directed) on 2026-09-15 — see §13.
**Scope:** Full repository — backend (`apps/api`), mobile (`apps/mobile`), admin (`apps/admin`), shared packages, infrastructure, docs.
**Method:** Pass 1: direct code inspection + 5 parallel deep-dive audits (security/authz, infrastructure/deployment, mobile release-engineering, external services, observability), cross-verified against each other and against actually running the code — not against CLAUDE.md's own self-reported changelog, which this audit treated as a claim to verify, not a fact. Pass 2: implemented every remaining P1/P2 finding from pass 1 that was completable without real credentials, external accounts, legal decisions, or a physical device — see §11. Pass 3: closed two of the P0 items pass 1/2 explicitly could not (Terms of Service/Privacy Policy content, account deletion) — see §13.
...
| GDPR/privacy (consent, deletion, export, retention) | Yes (found: absent; deletion+retention resolved in pass 3, §13; export still absent) | Direct inspection |
...
8. ~~No Terms of Service or Privacy Policy exists anywhere in the repo~~ — **resolved, see §13.** A static, unlinked disclaimer string ("By continuing, you accept VAYA's Terms of Use and Privacy Policy") used to be shown on the sign-in screen with nothing behind it; real Terms & Conditions and a Privacy Policy now exist (`docs/legal/`), are linked from the sign-in screen and the profile screen, and are rendered in-app in all 3 supported locales. **Not fully closed**: the content is AI-drafted from public research, not reviewed by licensed Tunisian counsel — see §13's caveats before treating it as final, and before submitting to either app store (both still need a real, permanently-hosted public URL, which is a separate step from having the content).
9. ~~No account-deletion or data-export mechanism exists~~ — **account deletion resolved, see §13; data export remains open.** A real `DELETE /users/me` now exists, soft-deletes and anonymizes the account, purges KYC files from storage, and is blocked while an active booking/ride exists. Data *export* (GDPR/INPDP portability) was intentionally left out of this pass's scope — it was not part of what was asked, and remains a real, separate gap.
...
- ~~No Terms of Service / Privacy Policy document exists~~ — content now exists (`docs/legal/`, §13); still needs licensed Tunisian counsel review and a real public hosting URL before store submission.
- ~~No account-deletion mechanism~~ — resolved (§13). Data *export* (portability) remains a separate, still-open gap.
...
11. [x] Draft Terms of Service and Privacy Policy (`docs/legal/`) and link them from the sign-in screen's disclaimer text — done, §13. [ ] Still needed: licensed Tunisian counsel review, and publishing the content at a real public URL for both store listings' Privacy Policy field (the in-app screens alone don't satisfy that requirement).
12. [x] Implement account deletion (`DELETE /users/me`, §13) — done. [ ] Data export (portability) was out of this pass's scope and remains a separate, still-open item; accept that gap explicitly or scope a follow-up before launch.
...
It is **not GO** because the remaining launch-blocking items are not code problems this audit can fix: no real infrastructure has ever been provisioned (Postgres/Redis/S3/Twilio/Sentry all need real accounts and credentials — `pnpm --filter @vaya/api preflight`, new in pass 2, will verify each one the moment real credentials exist), and nothing in this app has ever been verified on a real device. Terms of Service/Privacy Policy content and an account-deletion mechanism are no longer on this list — both now exist (§13) — but the ToS/Privacy content still needs a licensed Tunisian lawyer's review and a real public hosting URL before it's launch-final, which remains a human action this audit can't complete on its own.
...
## 13. Third pass — Terms & Conditions and account deletion (user-directed, separate session)

Scope: implement the two items §7/§8 flagged as needing a human — draft real Terms & Conditions and a Privacy Policy for VAYA specifically (Tunisia has no carpooling-specific law, so this needed real legal research, not boilerplate), and build a real account-deletion mechanism end-to-end.

**Legal drafting** — `docs/legal/terms-and-conditions.md` + `docs/legal/privacy-policy.md` (French, canonical/governing) + `docs/legal/README.md` (index, legal-review status, sources). Researched BlaBlaCar's actual ToS structure and legal framing (technical-intermediary status, cost-sharing cap distinct from a commercial fare) plus Tunisia's real legal context: Loi organique n°2004-63 + INPDP (personal data, with a heavier authorization regime for the biometric KYC selfie specifically), Loi n°2000-83 (e-commerce), Loi n°92-117 (consumer protection), and — the genuinely load-bearing finding — Loi n°2004-33's land-transport regime, under which *paid* carpooling is arguably "transport clandestin" absent the cost-sharing framing (a real, currently-enforced risk: a cited 700 TND fine + vehicle-seizure precedent against individuals, and the 2025 Bolt/Yassir/Heetch suspensions, the latter over tax/licensing for a genuinely commercial model, not carpooling). The CGU's cost-sharing article (Article 4) is written to match, in words, what `packages/domain/src/pricing`'s bounded-price formula already enforces server-side — not a separate, disconnected legal claim. In-app rendering: `apps/mobile/src/features/legal/content/{terms,privacy}.ts` (structured fr/en/ar content, French canonical) + `apps/mobile/app/legal/{terms,privacy}.tsx`, linked from `sign-in.tsx`'s previously-dead disclaimer text and from a new "Conditions et vie privée" section on the profile screen.

**Account deletion** — `DELETE /users/me` (`apps/api/src/modules/users/`). A real hard `DELETE FROM users` was confirmed unsafe for any user with real activity: `bookings.riderId`/`ratings.*UserId`/`messages.senderUserId` have no `onDelete` clause (Postgres would reject it), and cascading through `driver_profiles` would delete every ride a driver ever published — taking down every *other* rider's booking on those rides too. Implemented instead as soft-delete + real PII anonymization, mirroring the existing `suspendedAt`/`suspendedReason` pattern (new `users.deletedAt`/`deletionReason` columns, migration `0028_overconfident_gorilla_man.sql`, additive): blocked with a 409 while the user has an active booking (rider) or a published/full/in-progress ride (driver); on success, phone/email/googleId/fullName/avatarUrl are anonymized, every refresh token is revoked (`revokeAllRefreshTokensForUser`, new in `auth.service.ts`), and the `authenticate` hook (`app.ts`) now also rejects a still-live access token from a `deletedAt` account, mirroring the existing suspension check. **Real erasure, not just a DB flag**: `StorageAdapter` gained `remove`/`removeSecure` (implemented for both `LocalDiskStorageAdapter` and `S3StorageAdapter`), and `deleteUser` actually deletes every KYC document file and the avatar photo from storage, not just the DB row referencing them — matching what the Privacy Policy now promises. Mobile: `useDeleteMeMutation` (`state/api.ts`) + a two-step destructive confirmation flow on the profile screen (deliberately more friction than the existing single-step Logout confirmation, since this is irreversible), with a distinct blocked-state message for the 409 case.

**Tests added this pass**: `apps/api/src/modules/users/__tests__/delete-account.integration.test.ts` (real-Postgres integration test, same style as the existing `attach-phone.integration.test.ts`) — covers PII anonymization + refresh-token revocation, idempotency, the two 409-blocked cases (active rider booking, active driver ride), and — the part worth calling out — a genuine round-trip proof of file erasure (saves a real file via `storage.saveSecure`, confirms it's readable, calls `deleteUser`, confirms `storage.readSecure` now returns `null`), not just an assertion that a function was called. `apps/mobile/src/__tests__/profile-screen.snapshot.test.tsx`'s existing 5 snapshots regenerated for the new legal-section rows and delete-account affordance; diffed to confirm the changes were exactly the intended additions (`git diff` reviewed for both the new content markers and that no pre-existing row/copy was accidentally altered, not just re-recorded blindly).

**Genuinely verified, not assumed**: `pnpm typecheck`/`lint` clean across `apps/api` and `apps/mobile` after every change. The generated migration (`drizzle-kit generate`, not hand-written) is a clean 2-column additive `ALTER TABLE`, confirmed by reading the output SQL directly. Full `apps/mobile` test suite re-run: 267/276 passing, the same 9 pre-existing locale/ICU-date-drift + `Reanimated.FadeInDown` failures this file's own §5/§11 already documented, confirmed identical by stashing this session's changes and re-running against the untouched `HEAD` (same technique §11 used). API unit-testable suites (excluding `*.integration.test.ts`) re-run: same pre-existing `DATABASE_URL`-dependent failures as before, none touching the modules this pass changed (`users`, `auth`, `lib/storage`).

**Not verified, honestly flagged rather than assumed**: this sandbox had no Docker daemon reachable at session start; `dockerd` was started manually and did come up, but `docker compose up` still failed pulling images (`403 Forbidden` from the registry CDN through this environment's egress proxy) — so the new `delete-account.integration.test.ts` was written to the same real-Postgres convention every other integration test in this codebase uses, but **could not actually be executed against a live database in this session**, matching the exact limitation §6 and §11 already documented for every other integration suite here. The legal content is AI-drafted from public web research (BlaBlaCar's published terms, Tunisian statute text and secondary legal commentary, an INPDP procedures site) — substantive and specific to Tunisia and to VAYA's actual pricing/cancellation mechanisms, not generic boilerplate, but **not reviewed by a licensed Tunisian lawyer**, and the Arabic translation specifically has not been reviewed by a native legal-Arabic speaker. Both are tracked as explicit open checklist items in `docs/legal/README.md`, not glossed over. Also not done, and out of this pass's stated scope: data *export*/portability (distinct from deletion), and actually filing VAYA's INPDP declaration/authorization (a real operational action, not a code change).

**Updated verdict**: still **CONDITIONAL GO**, unchanged in kind — the remaining blockers are still credentials, infrastructure, and human legal/business decisions, not engineering. But the specific P0 items "no Terms of Service" and "no account-deletion mechanism" that were previously blocking are resolved at the content/mechanism level; what remains for them is a legal review and a public hosting step, not a build task.
