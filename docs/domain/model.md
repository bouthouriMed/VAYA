...
### `trips`
`id, bookingId (unique FK→bookings), rideId (FK), status (scheduled|driver_approaching|pickup|active|arriving|completed|no_show|cancelled), simulationStartedAt, pickupConfirmedAt, dropoffAt, completedAt (new — Phase 9), riderSettlementConfirmedAt, driverSettlementConfirmedAt, startedAt (new — live tracking), currentLat/Lng/HeadingDeg/SpeedMps/AccuracyM (new — live tracking, latest fix only, no history), locationUpdatedAt (new — live tracking)`. **Source of truth for trip-day execution state**, one row per accepted booking. This is what `bookings/pending.tsx` through `settlement.tsx` render (Phase 1 wired the screens to real data; Phase 9 added the actual `completed` transition; live tracking, see `docs/domain/live-tracking.md`, is the first real driver of the intermediate `driver_approaching`/`pickup`/`active`/`arriving` transitions). `completedAt` (nullable timestamp, additive) is set the moment a trip reaches `completed` and anchors the 24h rating-submission window (`packages/domain/src/rating/rating-window.ts`). The `current*`/`locationUpdatedAt` columns hold only the driver's latest reported GPS fix — overwritten in place on every update, never a history row per ping (deliberate retention minimization, `docs/domain/live-tracking.md`).
...
### `admin_users` / `audit_logs` / `analytics_events` / `reports` (new — live-tracking/admin/analytics initiative, see `docs/domain/admin-platform.md`)
`admin_users`: `id, email (unique), passwordHash, fullName, role (admin|superadmin), lastLoginAt`. A deliberately separate credential/auth system from `users` — see `docs/domain/admin-platform.md`. `audit_logs`: `id, adminUserId (FK), action (free text), targetType, targetId, reason, previousState/newState (jsonb), createdAt` — append-only, written by every mutating admin endpoint. `analytics_events`: `id, eventName (free text), userId (nullable FK, set null on delete), searchId, origin/destinationLabel+Lat+Lng, corridorKey (precomputed), desiredDepartureAt, seats, resultCount, matchTier, selectedRideId, durationMs, metadata (jsonb), createdAt` — the search-funnel + general `trackEvent` sink, one flat table, not event sourcing. `reports`: `id, reporterUserId (FK), reportedUserId/bookingId/tripId (nullable), category, description, status (open|investigating|resolved|dismissed), resolvedByAdminId, resolvedAt, resolutionNotes` — the first safety/moderation mechanism this codebase has had (previously zero, confirmed by the original audit and CLAUDE.md's Open Decision #6).
...
- **Verification status**: `pending → under_review → approved|rejected|resubmission_required`, `resubmission_required → pending`, on `driver_profiles` — extended (additive enum values) for the admin verification review workflow, see `docs/domain/verification-workflow.md`. Defined in `packages/domain/src/driver/verification-transitions.ts` (`canTransitionVerificationStatus`).
- **Tracking status** (new, live tracking — orthogonal to trip status, a GPS-feed-health state, not a ride-progress state): `not_started → starting → live ⇄ stale → unavailable`, terminal `completed` once the trip itself is terminal. Defined in `packages/domain/src/trip/tracking-status.ts` (`deriveTrackingStatus`), a pure function of trip status + fix age — never persisted as its own column, always derived fresh. See `docs/domain/live-tracking.md`.
...
| A driver's live GPS fix during a trip (new) | `trips` (latest fix only, no history — `docs/domain/live-tracking.md`) |
| Admin identity/credentials (new) | `admin_users` |
| Admin action history (new) | `audit_logs` |
| Search-funnel / product analytics events (new) | `analytics_events` |
| Safety/moderation reports (new) | `reports` |
