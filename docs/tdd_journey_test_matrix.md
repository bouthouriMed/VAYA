...
| M-021 | §5, §54, §62 | A driver-selected stop is NOT required for a feasible match (hard invariant) | `B.matching.route-passthrough-without-driver-stops` | FAIL (incorrect) — `route_passthrough` hard-requires stops both ends (matching.service.ts ~L634) |
...
| M-054 | §20 | Every request has a server-authoritative response deadline, visible to passenger immediately post-request and to driver inside the incoming request | `B.booking.deadline-visible-to-passenger`, `B.booking.deadline-visible-to-driver` (=EDGE-deadline-1/2) | FAIL (missing) — no `expiresAt`, client-only countdown |
...
| M-085 | §28 | Constraints (max detour, ETA impact, walk thresholds, deviation threshold, timing tolerance) are admin-configurable, not hardcoded | `B.admin-config.detour-threshold-configurable` | FAIL (partial) — pattern exists for pricing_configs, not for matching thresholds |
| M-086 | §28 | Not exposed as ordinary end-user configuration in v1 | `B.admin-config.thresholds-not-user-editable` | PASS (trivially, nothing is exposed) |
...
| M-113 | §39 | Notification fires for each of the 12 named lifecycle events with specific (non-generic) copy | `B.notifications.event-coverage-matrix` (12 sub-cases) | FAIL (partial) — 7/12 exist and specific; 4/12 missing (deadline-approaching, siblings-cancelled, passenger-onboard, route/ETA-changed); 2 conflated-but-functional |
...
| EDGE-054 | §54, §62 | No driver stop configured does not block a feasible match — stops are a preference signal, not a hard requirement | `B.matching.route-passthrough-without-driver-stops` (=M-021) | FAIL (incorrect) |
...
| INV-07 | §62 Matching | A driver-selected stop is not required for a feasible passenger match | `B.matching.route-passthrough-without-driver-stops` | FAIL (incorrect) |
...
- Spec sections 1–63 (core model, principles, publishing, search, ranking, request, pricing, capacity, tracking, lifecycle, cancellation, notifications, reviews): **all mapped**, IDs M-001…M-125. One reclassification this session: M-110 PASS → FAIL (incorrect), see above.
- Named edge cases §46–56: **all 11 mapped**, IDs EDGE-046…EDGE-056.
- Critical backend invariants §62: **all 9 mapped**, IDs INV-01…INV-09.
- Vertical journeys (spec's own §61 audit-journey framing + task's Journey 1–10): **all 10 mapped AND EXECUTED this session** as real HTTP journeys — 12 cases, 5 passed / 7 failed, every result verified live (see table above and `docs/tdd_journey_test_report.md`).
- Layer A (pure domain): 10 contract test files (3 pre-existing regression-locks + 7 new this session), covering existing-passenger-impact, auto-start, boarding, ETA confidence, live-corridor/deviation, no-show location corroboration, and the ride-level cancellation guard. 3 areas deliberately deferred (stop corridor-intent/joint-optimization, pedestrian-zone/no-stopping-feasibility rejection) — see `docs/tdd_journey_test_report.md`'s §9a.
